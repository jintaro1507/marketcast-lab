...
def _safe_float(v: object) -> float | None:
    """
    PostgREST は NUMERIC を文字列 ("100.5") で返す場合がある。
    float へ安全に変換する。None → None。NaN / Infinity は拒否する。
    """
    if v is None:
        return None
    if isinstance(v, bool):
        raise ValueError(f"bool は数値として扱えません: {v!r}")
    if isinstance(v, (int, float)):
        ...
        try:
            ...
            raise ValueError(f"NUMERIC 変換失敗: {v!r}") from None
    if not math.isfinite(f):
        raise ValueError(f"非有限値（NaN/Inf）は許可しません: {v!r}")
    return f
...
def _rows_with_value(db: WeeklyDB, week_id: str) -> dict[str, dict]:
    ...
    for r in (rows or []):
        try:
            ...
            raise WeeklyDBError(
                f"NUMERIC 変換失敗 ({r.get('asset_key')}): {e}"
            ) from None
        result[r["asset_key"]] = r
    return result
